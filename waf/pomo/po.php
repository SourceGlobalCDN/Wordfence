    ...
    class wfPO extends wfGettext_Translations
    {
        ...
        function read_entry($f, $lineno = 0)
        {
            ...
            while (true) {
                ...
                if (!$line) {
                    if (feof($f)) {
                        if (self::is_final($context)) {
                            break;
                        } else if (!$context) { // We haven't read a line and EOF came.
                            return null;
                        } else {
                            return false;
                        }
                ...
                } else if (preg_match('/^msgctxt\s+(".*")/', $line, $m)) {
                    if (self::is_final($context)) {
                        wfPO::read_line($f, 'put-back');
                        $lineno--;
                        break;
                ...
                } else if (preg_match('/^msgid\s+(".*")/', $line, $m)) {
                    if (self::is_final($context)) {
                        wfPO::read_line($f, 'put-back');
                        $lineno--;
                        break;
                ...
                } else if (preg_match('/^msgid_plural\s+(".*")/', $line, $m)) {
                    if ('msgid' !== $context) {
                        return false;
                    }
                ...
                } else if (preg_match('/^msgstr\s+(".*")/', $line, $m)) {
                    if ('msgid' !== $context) {
                        return false;
                    }
                    $context = 'msgstr';
                    $entry->translations = array(wfPO::unpoify($m[1]));
                } else if (preg_match('/^msgstr\[(\d+)\]\s+(".*")/', $line, $m)) {
