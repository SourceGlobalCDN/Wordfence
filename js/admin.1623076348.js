    ...
    if (!window['wordfenceAdmin']) { //To compile for checking: java -jar /usr/local/bin/closure.jar --js=admin.js --js_output_file=test.js
        window['wordfenceAdmin'] = {
                ...
                $('.wf-block-header-action-disclosure').each(function () {
                    ...
                    $(this).closest('.wf-block-header').on('click', function (e) {
                        ...
                        WFAD.ajax('wordfence_saveDisclosureState', {
                            name: $(this).closest('.wf-block').data('persistenceKey'),
                            state: !isActive
                        }, function () {
                        }, function () {
                        }, true);
            ...
            loadMoreIssues: function (callback, offset, limit, ignoredOffset, ignoredLimit) {
                ...
                WFAD.ajax('wordfence_loadIssues', {
                    offset: offset,
                    limit: limit,
                    ignoredOffset: ignoredOffset,
                    ignoredLimit: ignoredLimit
                }, function (res) {
                    WFAD.updateIssueCounts(res.issueCounts);
            ...
            updateIssueCounts: function (issueCounts) {
                ...
                if (newCount == 0) {
                    ...
                    if (existing.length == 0) {
                        var issue = $('#issueTmpl_noneFound').tmpl({
                            shortMsg: __('No new issues have been found.'),
                            id: 'no-issues-new'
                        });
                ...
                if (ignoredCount == 0) {
                    ...
                    if (existing.length == 0) {
                        var issue = $('#issueTmpl_noneFound').tmpl({
                            shortMsg: __('No issues have been ignored.'),
                            id: 'no-issues-ignored'
                        });
            ...
            colorboxError: function (errorMsg, isTokenError) {
                ...
                var prompt = $.tmpl(WordfenceAdminVars.tokenInvalidTemplate, {
                ...
                this.ajax('wordfence_addTwoFactor', {
                    ...
                    if (res.ok) {
                        if (mode == 'authenticator') {
                            ...
                            self.colorboxModalHTML((self.isSmallScreen ? '300px' : '440px'), __("Authentication Code"), message, {
                                onComplete: function () {
                                    jQuery('#wfTwoFactorQRCodeTable').qrcode({
                                        text: totpURL,
                                        width: (self.isSmallScreen ? 175 : 256),
                                        height: (self.isSmallScreen ? 175 : 256)
                                    });
                            ...
                            if (res.recoveryCodes.length > 0) {
                                ...
                                self.colorboxModalHTML((self.isSmallScreen ? '300px' : '400px'), __("Recovery Codes"), message, {
                                    onComplete: function () {
                                        jQuery('#wfTwoFactorDownload').on('click', function (e) {
                                            e.preventDefault();
                                            e.stopPropagation();
            ...
            setOption: function (key, value, successCallback, failureCallback) {
                var changes = {};
                changes[key] = value;
                this.ajax('wordfence_saveOptions', {
                    changes: JSON.stringify(changes),
                    page: WFAD.getParameterByName('page')
                }, function (res) {
                    if (res.success) {
            ...
            saveOptions: function (successCallback, failureCallback) {
                ...
                this.ajax('wordfence_saveOptions', {
                    changes: JSON.stringify(WFAD.pendingChanges),
                    page: WFAD.getParameterByName('page')
                }, function (res) {
                    if (res.success) {
                        typeof successCallback == 'function' && successCallback(res);
                ...
                menuItems: [
                    {
                        ...
                        primary: true,
                        disabled: $('#wf-save-changes').hasClass('wf-disabled'),
                        action: function () {
                            $('#wf-save-changes').trigger('click');
                        }
                    },
                    {
                        title: __('Cancel Changes'),
                        primary: false,
                        disabled: $('#wf-cancel-changes').hasClass('wf-disabled'),
                        action: function () {
                            $('#wf-cancel-changes').trigger('click');
                        }
                    },
                    {
                        title: __('Restore Defaults'),
                        primary: false,
                        disabled: $('#wf-restore-defaults').hasClass('wf-disabled'),
                        action: function () {
                            $('#wf-restore-defaults').trigger('click');
                        }
                    }
                ]
            });
...
!function (t) {
    ...
    var n = t.BlobBuilder || t.WebKitBlobBuilder || t.MozBlobBuilder || function (t) {
        var e = function (t) {
            ...
            }, i = n.prototype, a = o.prototype, r = t.FileReaderSync, c = function (t) {
                this.code = this[this.name = t]
            },
            l = "NOT_FOUND_ERR SECURITY_ERR ABORT_ERR NOT_READABLE_ERR ENCODING_ERR NO_MODIFICATION_ALLOWED_ERR INVALID_STATE_ERR SYNTAX_ERR".split(" "),
            s = l.length, u = t.URL || t.webkitURL || t, d = u.createObjectURL, f = u.revokeObjectURL, R = u,
            p = t.btoa, h = t.atob, b = t.ArrayBuffer, g = t.Uint8Array, w = /^[\w-]+:\/*\[?[\w\.:-]+\]?(?::[0-9]+)?/;
