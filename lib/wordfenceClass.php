...
class wordfence
{
    const ATTACK_DATA_BODY_LIMIT = 41943040;
    ...
    private static $userDat = false; //40MB
...
SQL
        ...
        if (!wfConfig::get('migration636_email_summary_excluded_directories')) {
            ...
            $key = array_search('wp-content/plugins/wordfence/tmp', $excluded_directories);
            if ($key !== false) {
                unset($excluded_directories[$key]);
            }
            $key = array_search('wp-content/wflogs', $excluded_directories);
            if ($key === false) {
                $excluded_directories[] = 'wp-content/wflogs';
            }
...
SQL
        ...
        if (!wfConfig::get('config701Migration', false)) {
            ...
            if ($settings == $limitedOptions) {
                wfConfig::set('scanType', wfScanner::SCAN_TYPE_LIMITED);
            } else if ($settings == $standardOptions) {
                wfConfig::set('scanType', wfScanner::SCAN_TYPE_STANDARD);
            } else if ($settings == $highSensitivityOptions) {
                wfConfig::set('scanType', wfScanner::SCAN_TYPE_HIGH_SENSITIVITY);
            } else {
                wfConfig::set('scanType', wfScanner::SCAN_TYPE_CUSTOM);
            }
...
<p><a href="javascript:WFAD.enableAllOptionsPage();" class="wf-btn wf-btn-primary wf-btn-callout-subtle">Enable "All Options" Page</a></p>', 'wfplugin_devalloptions');
    ...
    public static function status($level /* 1 has highest visibility */, $type /* info|error */, $msg)
    {
        ...
        if ($type != 'info' && $type != 'error') {
            error_log("Invalid status type: $type");
            return;
        }
        if (self::$printStatus) {
    ...
    public static function _refreshUpdateNotification($report = null, $useCachedValued = false)
    {
        ...
        if ($updatesNeeded) {
            ...
            for ($i = 0; $i < count($items); $i++) {
                if ($i > 0 && count($items) > 2) {
                    $message .= ', ';
                } else if ($i > 0) {
                    $message .= ' ';
                }
                if ($i > 0 && $i == count($items) - 1) {
                    $message .= __('and ', 'wordfence');
                }
                $message .= $items[$i];
            }
    ...
    public static function enqueueAJAXWatcher()
    {
        ...
        if (wfUtils::isAdmin() && !$wafDisabled) {
            wp_enqueue_style('wordfenceAJAXcss', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/wordfenceBox.css'), '', WORDFENCE_VERSION);
            wp_enqueue_script('wfi18njs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfi18n.js'), array(), WORDFENCE_VERSION);
            wp_enqueue_script('wordfenceAJAXjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/admin.ajaxWatcher.js'), array('jquery'), WORDFENCE_VERSION);
    ...
    {
        if (wfUtils::isAdmin()) {
            wp_enqueue_style('wf-adminbar', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/wf-adminbar.css'), '', WORDFENCE_VERSION);
            wp_enqueue_script('wordfenceDashboardjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfdashboard.js'), array('jquery'), WORDFENCE_VERSION);
            if (wfConfig::get('showAdminBarMenu')) {
                wp_enqueue_script('wordfencePopoverjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfpopover.js'), array('jquery'), WORDFENCE_VERSION);
    ...
    public static function lostPasswordPost($errors = null, $user = null)
    {
        ...
        if ($user === null) {
            ...
            if (is_array($user_login)) {
                $user_login = wfUtils::array_first($user_login);
            }
    ...
    { //Basic protection during WAF learning period
    ...
    private static function verifyTwoFactorIntermediateValues($userID, $twoFactorNonce)
    {
        ...
        if (empty($twoFactorNonce) || empty($twoFactorNonceTime)) {
            return false;
        } //Ensure the two factor nonce and time have been set
        if ($twoFactorNonce != $expectedNonce) {
            return false;
        } //Verify the nonce matches the expected

        $twoFactorUsers = wfConfig::get_ser('twoFactorUsers', array());
        if (!$twoFactorUsers || !is_array($twoFactorUsers)) {
            return false;
        } //Make sure there are two factor users configured
        foreach ($twoFactorUsers as &$t) { //Ensure the two factor nonce hasn't expired
            if ($t[0] == $user->ID && $t[3] == 'activated') {
                if (isset($t[5]) && $t[5] == 'authenticator') {
                    $graceTime = WORDFENCE_TWO_FACTOR_GRACE_TIME_AUTHENTICATOR;
                } else {
                    $graceTime = WORDFENCE_TWO_FACTOR_GRACE_TIME_PHONE;
                }
    ...
    { //As of php 5.4 we must denote passing by ref in the function definition, not the function call (as WordPress core does, which is a bug in WordPress).
        ...
        if (is_array($username) || is_array($passwd)) {
            return;
        }
    ...
    public static function authActionOld($username, $passwd)
    { //Code is identical to Newer function above except passing by ref ampersand. Some versions of PHP are throwing an error if we include the ampersand in PHP prior to 5.4.
        $lockout = wfBlock::lockoutForIP(wfUtils::getIP());
        if ($lockout !== false) {
            $lockout->recordBlock();
            $customText = wpautop(wp_strip_all_tags(wfConfig::get('blockCustomText', '')));
    ...
    public static function ajax_sendDiagnostic_callback()
    {
        ...
        if (function_exists('remove_filter')) {
            remove_filter('gettext', 'wordfence::_diagnosticsTranslationDisabler', 0);
        } //Remove for consistency. It's okay if it doesn't pre-4.7.0 since the call exits anyway.
        ...
        foreach ($blocks as $b) {
            ...
            switch ($b->type) {
                ...
                case wfBlock::TYPE_PATTERN:
                    ...
                    $components = array();
                    if (!empty($b->ipRange)) {
                        $components[] = __('IP Range', 'wordfence') . ' - ' . $b->ipRange;
                    }
                    if (!empty($b->userAgent)) {
                        $components[] = __('User Agent', 'wordfence') . ' - ' . $b->userAgent;
                    }
                    if (!empty($b->referrer)) {
                        $components[] = __('Referrer', 'wordfence') . ' - ' . $b->referrer;
                    }
                    if (!empty($b->hostname)) {
                        $components[] = __('Hostname', 'wordfence') . ' - ' . $b->hostname;
                    }
...
HTACCESS;
    ...
    { //Documented call for external interfacing.
    ...
    public static function ajax_dashboardShowMore_callback()
    {
        ...
        if ($grouping == 'ips') {
            $data = null;
            if ($period == '24h') {
                $data = $dashboard->ips24h;
            } else if ($period == '7d') {
                $data = $dashboard->ips7d;
            } else if ($period == '30d') {
                $data = $dashboard->ips30d;
            }
            ...
            $data = null;
            if ($period == 'success') {
                $data = $dashboard->loginsSuccess;
            } else if ($period == 'fail') {
                $data = $dashboard->loginsFail;
            }
    ...
    public static function wfFunc_view()
    {
        ...
        try {
            ...
            $fileSize = __('Unknown file size.', 'wordfence');
        }
    ...
    private static function wfFunc_testmem()
    {
        ...
        if ($configuredMax <= 0) {
            if ($last == 'g') {
                $configuredMax = $maxMemory * 1024;
            } else if ($last == 'm') {
                $configuredMax = $maxMemory;
            } else if ($last == 'k') {
                $configuredMax = $maxMemory / 1024;
            }
        ...
        } //Disables the WP error handler in somewhat of a hacky way
        ...
        while ($start <= $testLimit) {
            ...
            if (memory_get_usage(true) > $configuredMax) {
                break;
            }
            $index++;
        }
...
HTML;
    ...
    public static function admin_init()
    {
        ...
        wp_register_script('chart-js', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/Chart.bundle.min.js'), array('jquery'), '2.4.0');
        wp_register_script('wordfence-select2-js', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfselect2.min.js'), array('jquery', 'jquery-ui-tooltip'), WORDFENCE_VERSION);
        wp_register_style('wordfence-select2-css', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/wfselect2.min.css'), array(), WORDFENCE_VERSION);
        wp_register_style('wordfence-font-awesome-style', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/wf-font-awesome.css'), '', WORDFENCE_VERSION);
        ...
        if (isset($_GET['page']) && (preg_match('/^Wordfence/', @$_GET['page']) || ($_GET['page'] == 'WFLS' && wfOnboardingController::shouldShowNewTour(wfOnboardingController::TOUR_LOGIN_SECURITY)))) {
            ...
            wp_enqueue_style('wordfence-main-style', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/main.css'), '', WORDFENCE_VERSION);
            ...
            wp_enqueue_script('jquery.wftmpl', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/jquery.tmpl.min.js'), array('jquery'), WORDFENCE_VERSION);
            ...
            wp_enqueue_script('jquery.wfdataTables', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/jquery.dataTables.min.js'), array('jquery'), WORDFENCE_VERSION);
            wp_enqueue_script('jquery.qrcode', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/jquery.qrcode.min.js'), array('jquery'), WORDFENCE_VERSION);
            //wp_enqueue_script('jquery.tools', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/jquery.tools.min.js'), array('jquery'));
            wp_enqueue_script('wfi18njs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfi18n.js'), array(), WORDFENCE_VERSION);
            wp_enqueue_script('wordfenceAdminExtjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfglobal.js'), array('jquery'), WORDFENCE_VERSION);
            wp_enqueue_script('wordfenceAdminjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/admin.js'), array('jquery', 'jquery-ui-core', 'jquery-ui-menu'), WORDFENCE_VERSION);
            wp_enqueue_script('wordfenceDropdownjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfdropdown.js'), array('jquery'), WORDFENCE_VERSION);
            self::setupAdminVars();
            ...
            wp_enqueue_script('wordfenceAdminExtjs', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/wfglobal.js'), array('jquery'), WORDFENCE_VERSION);
            wp_enqueue_style('wordfence-font-awesome-style');
            wp_enqueue_style('wordfence-global-style', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/wf-global.css'), '', WORDFENCE_VERSION);
            self::setupAdminVars();
        }
        ...
        <div id="<?php echo esc_attr($id) ?>" class="notice notice-warning">
            <p>
                ...
                <a target="_blank" rel="noopener noreferrer"
                   href="<?php echo wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_BLOCKING_FULL_SITE) ?>"><?php esc_html_e('Learn More', 'wordfence') ?></a>
                <a class="wf-btn wf-btn-default wf-btn-sm wf-dismiss-link" href="#"
                   onclick="<?php echo esc_attr('wordfenceExt.hideNoticeForUser(' . json_encode($id) . '); return false;') ?>"><?php esc_html_e('Dismiss', 'wordfence') ?></a>
            </p>
        </div>
        <?php
    }
    ...
    public static function admin_menus()
    {
        ...
        add_menu_page('Wordfence', "Wordfence{$dashboardExtra}", 'activate_plugins', 'Wordfence', 'wordfence::menu_dashboard', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . 'images/wordfence-logo.svg');
    }
...
JQUERY;
    ...
    {
        wp_enqueue_style('wordfence-jquery-ui-css', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/jquery-ui.min.css'), array(), WORDFENCE_VERSION);
        wp_enqueue_style('wordfence-jquery-ui-structure-css', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/jquery-ui.structure.min.css'), array(), WORDFENCE_VERSION);
        wp_enqueue_style('wordfence-jquery-ui-theme-css', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/jquery-ui.theme.min.css'), array(), WORDFENCE_VERSION);
        wp_enqueue_style('wordfence-jquery-ui-timepicker-css', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/jquery-ui-timepicker-addon.css'), array(), WORDFENCE_VERSION);

        wp_enqueue_script('wordfence-timepicker-js', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/jquery-ui-timepicker-addon.js'), array('jquery', 'jquery-ui-datepicker', 'jquery-ui-slider'), WORDFENCE_VERSION);
        wp_enqueue_script('wordfence-knockout-js', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/knockout-3.5.1.js'), array(), WORDFENCE_VERSION);
        wp_enqueue_script('wordfence-live-traffic-js', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('js/admin.liveTraffic.js'), array('jquery', 'jquery-ui-tooltip'), WORDFENCE_VERSION);
...
SQL
    ...
    public static function addDashboardWidget()
    {
        if (wfUtils::isAdmin() && (is_network_admin() || !is_multisite()) && wfConfig::get('email_summary_dashboard_widget_enabled')) {
            wp_enqueue_style('wordfence-activity-report-widget', "https://source.ahdark.com/wordpress/plugin/wordfence/" . WORDFENCE_VERSION . "/" . wfUtils::versionedAsset('css/activity-report-widget.css'), '', WORDFENCE_VERSION);
