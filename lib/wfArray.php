...
class wfArray
{
    ...
    { //associative array with keys that match those given to constructor
    ...
    { //If you alternately call push and shift you must periodically call collectGarbage() or ->data will keep growing
        $arr = array();
        if (wfUtils::strlen($this->data) < 1) {
            return null;
        }
        if ($this->shiftPtr == wfUtils::strlen($this->data)) {
            return null;
        }
        foreach ($this->keys as $key) {
    ...
    { //only call collectGarbage if you're alternating between pushes and shifts and never emptying the array.
    ...
    { //Rather call this instead of collect garbage because it's way more mem efficient.
        $this->data = "";
        $this->shiftPtr = 0;
        $this->size = 0;
    }
