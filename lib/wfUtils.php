...
class wfUtils
{
    ...
    public static function getIPGeo($IP)
    { //Works with int or dotted
    ...
    public static function errorsOn()
    {
        ...
        if (class_exists('wfScan')) {
            wfScan::$errorHandlingOn = true;
        }
    }

    public static function fileOver2Gigs($file)
    { //Surround calls to this func with try/catch because fseek may throw error.
        $fh = @fopen($file, 'r');
        if (!$fh) {
            return false;
        }
    ...
    {
        require(dirname(__FILE__) . '/wfBulkCountries.php');
    ...
    { // takes a pattern using asterisks as wildcards, turns it into regex and checks it against the visitor UA returning true if blocked
