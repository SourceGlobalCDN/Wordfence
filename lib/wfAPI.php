...
class wfAPI
{
    ...
    { // In the form '/something.bin' without quotes
    ...
    public function call($action, $getParams = array(), $postParams = array(), $forceSSL = false, $timeout = 900)
    {
        ...
        if (isset($dat['_isPaidKey'])) {
            ...
            if (!isset($dat['errorMsg'])) {
                ...
                if (isset($dat['_autoRenew'])) {
                    wfConfig::set('premiumAutoRenew', wfUtils::truthyToInt($dat['_autoRenew']));
                } else {
                    wfConfig::remove('premiumAutoRenew');
                }
                if (isset($dat['_nextRenewAttempt'])) {
                    wfConfig::set('premiumNextRenew', time() + $dat['_nextRenewAttempt'] * 86400);
                } else {
                    wfConfig::remove('premiumNextRenew');
                }
                if (isset($dat['_paymentExpiring'])) {
                    wfConfig::set('premiumPaymentExpiring', wfUtils::truthyToInt($dat['_paymentExpiring']));
                } else {
                    wfConfig::remove('premiumPaymentExpiring');
                }
                if (isset($dat['_paymentExpired'])) {
                    wfConfig::set('premiumPaymentExpired', wfUtils::truthyToInt($dat['_paymentExpired']));
                } else {
                    wfConfig::remove('premiumPaymentExpired');
                }
                if (isset($dat['_paymentMissing'])) {
                    wfConfig::set('premiumPaymentMissing', wfUtils::truthyToInt($dat['_paymentMissing']));
                } else {
                    wfConfig::remove('premiumPaymentMissing');
                }
                if (isset($dat['_paymentHold'])) {
                    wfConfig::set('premiumPaymentHold', wfUtils::truthyToInt($dat['_paymentHold']));
                } else {
