...
class wfConfig
{
    ...
    ); //enables if we detect a write fail so we don't keep calling stat()
    ...
    private static $diskCacheDisabled = false;
...
';
    ...
    public static function createLock($name, $timeout = null)
    { //Our own version of WP_Upgrader::create_lock that uses our table instead
        global $wpdb;
    ...
    public static function removeCodeExecutionProtectionForUploads()
    {
        ...
        if (file_exists($uploads_htaccess_file_path)) {
            ...
            if (preg_match(self::$_disable_scripts_regex, $htaccess_contents)) {
                ...
                } else if (@file_put_contents($uploads_htaccess_file_path, $htaccess_contents, LOCK_EX) === false) {
                    throw new wfConfigException($error_message);
                }
            }
        }
