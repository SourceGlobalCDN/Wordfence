    ...
    <?php else: ?>
        <table class="wfls-table wfls-table-striped wfls-table-header-separators wfls-table-expanded wfls-no-bottom">
            ...
            <?php foreach ($users as $user): ?>
                <tr>
                    <th scope="row"><a
                                href="<?php echo esc_attr(get_edit_user_link($user->user_id)) ?>#wfls-user-settings"><?php echo esc_html($user->user_login) ?></a>
                    </td>
                    <td>
            ...
            <?php if ($page != 1 || !$lastPage): ?>
                <tr>
                    <td colspan="2" class="wfls-center">
                        <?php if ($page > 1): ?>
                            <a href="<?php echo esc_attr(add_query_arg($pageKey, $page - 1) . "#$stateKey") ?>"><span
