...
END
    ...
    {
        ?>
        <div id="<?php echo esc_attr(Controller_Notices::PERSISTENT_NOTICE_WOOCOMMERCE_INTEGRATION) ?>"
             class="notice notice-warning is-dismissible wfls-persistent-notice">
            <p>
    ...
    public function _login_enqueue_scripts()
    {
        ...
        if ($useCAPTCHA) {
            wp_enqueue_script('wordfence-ls-recaptcha', 'https://www.recaptcha.net/recaptcha/api.js?render=' . urlencode(Controller_Settings::shared()->get(Controller_Settings::OPTION_RECAPTCHA_SITE_KEY)));
        }
    ...
    public function _admin_enqueue_scripts($hookSuffix)
    {
        if (isset($_GET['page']) && $_GET['page'] == 'WFLS') {
            wp_enqueue_script('wordfence-ls-jquery.qrcode', Model_Asset::js('jquery.qrcode.min.js', 'modules/login-security/js/'), array('jquery'), WORDFENCE_LS_VERSION);
            wp_enqueue_script('wordfence-ls-jquery.tmpl', Model_Asset::js('jquery.tmpl.min.js', 'modules/login-security/js/'), array('jquery'), WORDFENCE_LS_VERSION);
            wp_enqueue_script('wordfence-ls-jquery.colorbox', Model_Asset::js('jquery.colorbox.min.js', 'modules/login-security/js/'), array('jquery'), WORDFENCE_LS_VERSION);
            if (Controller_Permissions::shared()->can_manage_settings()) {
                wp_enqueue_style('wordfence-ls-jquery-ui-css', Model_Asset::css('jquery-ui.min.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
                wp_enqueue_style('wordfence-ls-jquery-ui-css.structure', Model_Asset::css('jquery-ui.structure.min.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
                wp_enqueue_style('wordfence-ls-jquery-ui-css.theme', Model_Asset::css('jquery-ui.theme.min.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
            }
            wp_enqueue_script('wordfence-ls-admin', Model_Asset::js('admin.js', 'modules/login-security/js/'), array('jquery'), WORDFENCE_LS_VERSION);
            if (!WORDFENCE_LS_FROM_CORE) {
                wp_register_script('chart-js', Model_Asset::js('Chart.bundle.min.js', 'modules/login-security/js/'), array('jquery'), '2.4.0');
                wp_register_script('wordfence-select2-js', Model_Asset::js('wfselect2.min.js', 'modules/login-security/js/'), array('jquery'), WORDFENCE_LS_VERSION);
                wp_register_style('wordfence-select2-css', Model_Asset::css('wfselect2.min.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
            }
            wp_enqueue_script('chart-js');
            wp_enqueue_script('wordfence-select2-js');
            wp_enqueue_style('wordfence-select2-css');
            wp_enqueue_style('wordfence-ls-admin', Model_Asset::css('admin.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
            wp_enqueue_style('wordfence-ls-colorbox', Model_Asset::css('colorbox.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
            wp_enqueue_style('wordfence-ls-ionicons', Model_Asset::css('ionicons.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
            if (!WORDFENCE_LS_FROM_CORE) {
                wp_enqueue_style('wordfence-ls-font-awesome', Model_Asset::css('font-awesome.css', 'modules/login-security/css/'), array(), WORDFENCE_LS_VERSION);
            }
                ...
                <tr id="wordfence-ls">
                    ...
                    <td>
                        <?php if ($userAllowed2fa): ?>
                            <p>
                                <strong><?php echo $lockedOut ? esc_html__('Locked Out', 'wordfence-2fa') : ($has2fa ? esc_html__('Active', 'wordfence-2fa') : esc_html__('Inactive', 'wordfence-2fa')); ?>
                                    :</strong>
                                <?php echo $lockedOut ?
                                ...
                                <a href="<?php echo Controller_Support::esc_supportURL(Controller_Support::ITEM_MODULE_LOGIN_SECURITY_2FA); ?>"
                                   target="_blank"
                                   rel="noopener noreferrer"><?php esc_html_e('Learn More', 'wordfence-2fa'); ?></a>
                            </p>
                            <?php if (!$has2fa && $inGracePeriod): ?>
                                <p><strong><?php echo sprintf($viewerIsUser ?
                                            ...
                                            , Controller_Time::format_local_time('F j, Y g:i A', $requiredAt)) ?></strong>
                                </p>
                            <?php endif ?>
                            <?php if ($has2fa || $viewerIsUser): ?><p><a href="<?php echo esc_url($manageURL); ?>"
                                                                         class="button"><?php echo(Controller_Users::shared()->has_2fa_active($user) ? esc_html__('Manage 2FA', 'wordfence-2fa') : esc_html__('Activate 2FA', 'wordfence-2fa')); ?></a>
                                </p><?php endif ?>
                        <?php endif ?>
                        <?php if ($viewerCanManage2fa): ?>
                            <?php if (!$userAllowed2fa): ?>
                                <p><strong><?php esc_html_e('Disabled', 'wordfence-2fa'); ?>
                                        :</strong> <?php esc_html_e('Two-factor authentication is not currently enabled for this account type. To enable it, visit the Wordfence 2FA Settings page.', 'wordfence-2fa'); ?>
                                    <a href="#"><?php esc_html_e('Learn More', 'wordfence-2fa'); ?></a></p>
                            <?php endif ?>
                            <?php if ($lockedOut): ?>
                            ...
                            <?php endif ?>
                            <p>
                                <a href="<?php echo esc_url(is_multisite() ? network_admin_url('admin.php?page=WFLS#top#settings') : admin_url('admin.php?page=WFLS#top#settings')); ?>"
                                   class="button"><?php esc_html_e('Manage 2FA Settings', 'wordfence-2fa'); ?></a>
                            </p>
                        <?php endif ?>
                    </td>
                </tr>
                ...
                if (Controller_Users::shared()->has_2fa_active($user)) { //CAPTCHA enforcement skipped for users with 2FA active
                    ...
                    if ($encryptedNonce) {
                        ...
                        if (!defined('WORDFENCE_LS_CAPTCHA_CACHE')) {
                            define('WORDFENCE_LS_CAPTCHA_CACHE', (string)$jwt);
                        }
                    }
